const getColors = (svgString: string): 
{ fill: string[], stroke: string[], stopColor: string[] } => {
    ...
    const fillMatch = svgString.match(/fill="([^"]+)"/gi);
    const strokeMatch = svgString.match(/stroke="([^"]+)"/gi);
    const stopColorMatch = svgString.match(/stop-color="([^"]+)"/gi);
